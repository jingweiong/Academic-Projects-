/**
 * @file parallel_plaintext.c
 * @brief This file contains the parallel implementation of the DES algorithm using plaintext parallelism.
 * @details The third parallel scheme divides the entire message into
            equal partitions, one for each processor. Each processor
            then breaks its own partition into 64-bit blocks to actually
            implement the DES rounds. This scheme is based on the
            actual length of the plaintext, so it is plaintext based.
 * @author Ong Jing Wei
 * @date 2023-10-28
*/
...
int main() {
    ...
    int blocksPerProcessor = pt_count / (8 * NUMPROCESSORS);
    ...
    #pragma omp parallel num_threads(NUMPROCESSORS)
    {
        ...
        int startBlock = myThread * blocksPerProcessor;
        int endBlock = (myThread + 1) * blocksPerProcessor;

        for (int block = startBlock; block < endBlock; block++) {
            // Encryption process for each block
            char* cipher = encryption(pt[block], roundKeys);
